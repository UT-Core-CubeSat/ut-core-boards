...
The communications board should be able to provide UHF uplink/downlink and S-band downlink capbility. This board should be able to communicate with both UHF and S-band ground stations, using UHF for telemetry data and S-band for payload data downlink. 
...
This board needs to communicate properly with both UHF and S-band ground stations. UHF should be able to succesfully transmit and recieve while S-band should transmit payload data with no errors. It is still not decided wheather or not UHF and S-band will be on the same board. This decision would need to be made based on RF layout, spacing, routing, and system complexity. 
